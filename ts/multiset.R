...
for(i in phi){
  ...
  for(j in 1:final.iter){
    maxlow[j,1] <- quantile(abslowmle[j,], 0.975)
    maxlow[j,2] <- quantile(abslowmm[j,], 0.975)
    maxlow[j,3] <- quantile(absloweq[j,], 0.975)
    maxupp[j,1] <- quantile(absuppmle[j,], 0.975)
    maxupp[j,2] <- quantile(absuppmm[j,], 0.975)
    maxupp[j,3] <- quantile(absuppeq[j,], 0.975)
    minlow[j,1] <- quantile(abslowmle[j,], 0.025)
    minlow[j,2] <- quantile(abslowmm[j,], 0.025)
    minlow[j,3] <- quantile(absloweq[j,], 0.025)
    minupp[j,1] <- quantile(absuppmle[j,], 0.025)
    minupp[j,2] <- quantile(absuppmm[j,], 0.025)
    minupp[j,3] <- quantile(absuppeq[j,], 0.025)
  }
  ...
  plot2 <- ggplot(df.abs, aes(x = id, group = Type)) +  # Plot for lower bound
    geom_line(aes(y = low, col = Type, linetype = Type), lwd=0.75) +
    geom_line(aes(y = minlow, col = Type), linetype = "dotdash", lwd=0.75) +
    geom_line(aes(y = maxlow, col = Type), linetype = "dotdash", lwd=0.75) +
    scale_x_continuous(trans="log10") +
  ...
  plot3 <- ggplot(df.abs, aes(x = id, group = Type)) +  # Plot for upper bound
    geom_line(aes(y = upp, col = Type, linetype = Type), lwd=0.75) +
    geom_line(aes(y = minupp, col = Type), linetype = "dotdash", lwd=0.75) +
    geom_line(aes(y = maxupp, col = Type), linetype = "dotdash", lwd=0.75) +
    scale_x_continuous(trans="log10") +
    scale_y_continuous(trans="log10", limits = c(0.0001,1.95)) +
    labs(x = "Iterations", y = "Absolute difference from true upper bound") +
    # ylim(0.03, 0.35) +
...
plot.list <- ggarrange(plots[[1]], plots[[2]], plots[[3]], plots[[4]],
                       ...
                       plots[[9]], plots[[10]], plots[[11]], plots[[12]],
                       nrow = 3, ncol = 4, common.legend = TRUE, legend="bottom")
ggexport(plot.list, filename = "signplot.pdf", width = 30, height = 14)